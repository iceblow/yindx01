		...
		<div class="row">
				...
				<div class="form-group">
					...
					<div class="col-sm-2">
						<select data-rel="tooltip" id="proselect" onchange="changepro()"
							...
							title="省份" data-placement="bottom">
							<option value="">请选择</option>
							<c:forEach var="p" items="${pro }">
								<c:if test="${(city.provienceid==p.id) }">
					...
					<div class="col-sm-2">
						<select data-rel="tooltip" id="cityselect" type="text"
							class="col-sm-10 searchKey" name="city" title="城市"
							data-placement="bottom" onclick="cityselects()">
							<option value="${city.cityid }">${city.cityName }</option>
						</select>
					</div>
					<label Style="text-align: center;"
						class="col-sm-1 control-label no-padding-right" for="form-field-6">区域</label>
					<div class="col-sm-2">
			...
			<div class="col-xs-12">
				<div class="form-group">
					...
					<div class="col-sm-2" id="hiddenDIV">
						<c:if test="${banner.type==0 }">
							<input class="col-sm-10 searchKey" id="contentid"
								name="contentid" value="${banner.contentid }" class="searchKey"
								title="内容" data-placement="bottom" />
						</c:if>
						<c:if test="${banner.type==1 }">
							<select data-rel="tooltip" type="text"
								class="col-sm-10 searchKey" id="contentid" name="contentid"
								title="内容" data-placement="bottom">
								<option value="">请选择</option>
								<c:forEach items="${cmp }" var="c">
									<option value="${c.companyid }"
										<c:if test="${banner.contentid==c.companyid }">selected</c:if>>${c.name }</option>
								</c:forEach>
							</select>
						</c:if>
						<c:if test="${banner.type==2 }">
							<select data-rel="tooltip" type="text"
								...
								<c:forEach items="${aunt }" var="a">
									<option value="${c.auntid }"
										<c:if test="${banner.contentid==a.auntid }">selected</c:if>>${a.realName }</option>
								</c:forEach>
							</select>
						</c:if>
					</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-8">
						<div id="dropzone" class="col-sm-2 dropzone" data-img="${img }">
							<img data-dz-thumbnail width="184px" height="200px" src="${img }"
								id="changeimg" />

						</div>
						<input value="还原" onclick="returnOldPic()"
			...
			<div class="col-xs-12">
				<div class="form-group">
					<label Style="text-align: center;"
						class="col-sm-1 control-label no-padding-right" for="form-field-6">key</label>
					<div class="col-sm-2">
			...
			$
					.ajax({
						...
						success : function(result) {
							...
							$('#hiddenDIV').empty();
							var select = '<select data-rel="tooltip" type="text" class="col-sm-10 searchKey" id="contentid" name="contentid" title="内容" data-placement="bottom" >';
			...
			$.ajax({
				...
				success : function(msg) {
					...
					for (var i = 0; i < msg.list.length; i++) {
						html += '<option value="' + msg.list[i].name
								+ '" onclick="areaselect(' + msg.list[i].id
