...
<script>
    var mapObj;
    setItem('server_url','../auth/go?uri=/wechat/serves/piju');
    ...
    var uncle = new Vue({
        ...
        data: {
            ...
            longitude: getItem('user_lng'), latitude: getItem('user_lat'), serverid: 10, thirdids: ''
        },
        emulateJSON: true,
        mounted: function() {
        },
                ...
                this.$http.get(createUrl(this.serveUrl, {}, true))
                        .then(function(response) {
                            if (response.data.c == 1) {
                                var jsonData = JSON.parse(response.data.r);
//                                console.log(jsonData)
                                var server=jsonData.firstList[2].secondList[2];
