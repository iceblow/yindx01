            ...
            <div class="list-block" style="margin-top: 20px;margin-bottom: 60px">
                <ul>
                    ...
                    <li>
                        <div class="item-content">
                            <div class="item-inner">
                                <div class="item-title label server_title">服务类型</div>
                                <div class="item-input">
                                    <a class="server_btn button" v-on:click="getId(1,$event)">晚餐</a>
                                    <a class="server_btn button" v-on:click="getId(2,$event)">午餐</a>
                                    <a class="server_btn button" v-on:click="getId(3,$event)">早餐</a>
                                </div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div class="item-content">
                            <div class="item-inner">
                                <div class="item-title label server_title">服务类别</div>
                                <div class="item-input">
                                    <a class="server_btn button" v-on:click="getId(4,$event)">买菜</a>
                                    <a class="server_btn button" v-on:click="getId(5,$event)">做饭</a>
                                    <a class="server_btn button" v-on:click="getId(6,$event)">洗碗</a>
                                </div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <div class="item-content">
                            <div class="item-inner">
                                <div class="item-title label server_title">饭菜方面</div>
                                <div class="item-input open-popup" data-popup=".popup-about">
                                    <div class="server_more_box" v-if="feelList.length != 0" v-for="item in feelList">
                                        <p>{{item.name}}</p>
                                    </div>
                                    <div class="uncle_more text_placeholder" v-if="feelList.length == 0">非必填项</div>
                                </div>
                            </div>
                        </div>
                    </li>
                            ...
                            <ul>
                                <li v-for="(items,x) in list">
                                    ...
                                    <div class="item-content">
                                        <div class="item-inner">
                                            <div class="item-input">
                                                <a class="server_btn button" :id="item.id" v-for="(item,y) in items.secondList" v-on:click="chose_shoe(item.id,item.name,$event)">{{item.name}}</a>
                                            </div>
                                        </div>
                                    </div>
                                </li>
...
<script>
    setItem('server_url','../auth/go?uri=/wechat/serves/zuofan');
    ...
    var uncle = new Vue({
        ...
        data: {
            ...
            userInfo: [], longitude: lng, latitude: lat, serverid:this.serverid
        },
        emulateJSON: true,
        mounted: function() {
        },
        methods: {
            getCustomers: function() {
                this.$http.get(createUrl(this.apiUrl, {longitude: this.longitude, latitude: this.latitude, serverid: service.serverid}, false))
...
//                                console.log(jsonData)
                                var server=jsonData.firstList[1].secondList[2];
...
//                                console.log(jsonData)
            ...
            getId:function (id,e) {
                if(!this.foodselect.contains(id)){
                    this.foodselect.push(id);
                    $(e.currentTarget).addClass('btn_active');
                }else {
                    this.foodselect.removeByValue(id);
                    $(e.currentTarget).removeClass('btn_active');
                }
                console.log(this.foodselect.join(','));
            },
            ...
            sendData:function () {
                ...
                var bookDate={
                    ...
                    third_json:JSON.stringify(this.feelList),
                    foodselect:this.foodselect.join(','),
                };
...
//                image: "http://ofc6sbq4f.bkt.clouddn.com/25D4C9789AD3EA58E91AC5EFF4E783B3",
    ...
    var chose_shoe = new Vue({
        ...
        data:{
            ...
            ids_s:{id:38},
            ids_m:{id:41},
            feelList:service.feelList,
        },
        methods:{
            chose_shoe:function (id,type,e) {
                if(id < 41){
                    if(!this.ids_s.id != id){
                        this.ids_s.id = id;
                        service.feelList.push({
                            id:id,
                            name:type
                        })
                    }
                ...
                for(var i=0;i<service.feelList.length;i++){
                    $('#'+service.feelList[i].id).addClass('btn_active');
                }
                console.log(service.feelList)
            },
        }
