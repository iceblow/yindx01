...
<body>
	...
	<script type="text/javascript">
		...
		function changestatus(cellvalue, options, rowObject) {
			var html = '';
 			html += '<button class="btn btn-xs btn-primary" onclick="deal(\''
 				+ rowObject.orderid + '\')" style="margin-right:10px;">解决</button>';
 			html += '<button class="btn btn-xs btn-primary" onclick="complete(\''
 	 			+ rowObject.orderid + '\')" style="margin-right:10px;">完成</button>';
			return html;
		}
		function deal(id){
			$.ajax({
		...
		function stateformat(cellvalue, options, rowObject) {
			var html = '';
			if (rowObject.state == 7) {
				html = '已退单';
			} else if (rowObject.state == 8) {
				html = '被拒单';
			} else {
				html = '无人抢单';
			}
